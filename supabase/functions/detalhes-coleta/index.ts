...
serve(async (req) => {
  ...
  try {
    ...
    console.log("📥 Buscando detalhes da coleta:", idColeta);

    // Validação
    if (!idColeta || idColeta === 0) {
      return new Response(
        JSON.stringify({ success: false, error: "idColeta é obrigatório" }),
    ...
    // Buscar detalhes da coleta na API DB Frete
    console.log("🔍 Buscando coleta na API DB Frete...");
    const coletaResponse = await fetch(`${dbfreteColetaUrl}/${idColeta}`, {
      method: 'GET',
      headers: {
    ...
    if (!coletaResponse.ok) {
      const errorText = await coletaResponse.text();
      console.error("❌ Erro ao buscar coleta:", coletaResponse.status, errorText);
      
      if (coletaResponse.status === 401) {
        throw new Error("Token expirado ou inválido");
      }
      if (coletaResponse.status === 404) {
        throw new Error(`Coleta ${idColeta} não encontrada`);
      }
      throw new Error(`Erro ao buscar coleta: ${errorText}`);
    }

    const coletaData = await coletaResponse.json();
    console.log("✅ Detalhes da coleta obtidos com sucesso");
    ...
    console.error("❌ Erro na função detalhes-coleta:", error);
    const errorMessage = error instanceof Error ? error.message : "Erro desconhecido";
    
    // Salvar erro na tabela erros
    try {
      await supabase.from("erros").insert({
        titulo: "Erro ao buscar detalhes da coleta",
        descricao: errorMessage,
        categoria: "coleta",
        pagina: "/detalhes-coleta",
        nivel: "error",
        tipo: "api",
        dados_extra: {
