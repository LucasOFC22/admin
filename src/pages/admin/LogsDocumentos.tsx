...
import { LogDocumento } from '@/types/allLogs';
...
const LogsDocumentos = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['logs-documentos', filters, currentPage, pageSize],
    queryFn: async () => {
      ...
      return { logs: data as LogDocumento[], total: count || 0 };
    }
  });
  ...
  const columns = [
    ...
    { key: 'documento_id', label: 'ID Documento' },
    { key: 'solicitacao_id', label: 'ID Solicitação' }
  ];
  ...
  return (
    <PermissionGuard 
      permissions="admin.logs-documentos.visualizar"
      showMessage={true}
    >
      <div className="space-y-6 p-6">
          <LogsHeader
          title="Logs de Documentos"
          description="Histórico de alterações em documentos e solicitações"
          icon={<FileText className="h-6 w-6" />}
          onRefresh={refetch}
          data={logs}
          filename="logs_documentos"
        />
