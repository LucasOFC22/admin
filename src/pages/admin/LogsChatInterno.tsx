/**
 * Página de Logs de Chat Interno
 */
...
import { LogChatInterno } from '@/types/allLogs';
...
const LogsChatInterno = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['logs-chat-interno', filters, currentPage, pageSize],
    queryFn: async () => {
      ...
      return { logs: data as LogChatInterno[], total: count || 0 };
    }
  });
  ...
  return (
    <PermissionGuard
      permissions="admin.logs-chat-interno.visualizar"
      showMessage={true}
    >
      <div className="space-y-6 p-6">
        <LogsHeader
          title="Logs de Chat Interno"
          description="Histórico de atividades no chat interno"
          icon={<MessageCircle className="h-6 w-6" />}
          onRefresh={refetch}
          data={logs}
          filename="logs_chat_interno"
        />
...
export default LogsChatInterno;
