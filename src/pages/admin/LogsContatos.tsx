...
import { LogContato } from '@/types/allLogs';
...
const LogsContatos = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['logs-contatos', filters, currentPage, pageSize],
    queryFn: async () => {
      ...
      return { logs: data as LogContato[], total: count || 0 };
    }
  });
  ...
  return (
    <PermissionGuard
      permissions="admin.logs-contatos.visualizar"
      showMessage={true}
    >
      <div className="space-y-6 p-6">
        <LogsHeader
          title="Logs de Contatos"
          description="Histórico de alterações em contatos"
          icon={<Contact className="h-6 w-6" />}
          onRefresh={refetch}
          data={logs}
          filename="logs_contatos"
        />
